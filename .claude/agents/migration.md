...
- Never apply a migration to a live project without the user's explicit confirmation — drafting and reviewing SQL is your job, running `supabase db push` against a real database is a decision for the user. Once confirmed, see CLAUDE.md's "Database (Supabase)" section for the actual link/auth mechanics (personal access token, `--linked` flag, etc.) — `supabase login`'s browser flow doesn't work non-interactively.
